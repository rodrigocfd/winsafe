...
/// Implements all window event traits.
pub(in crate::gui) struct BaseWndEvents {
	wnd_ty: WndTy,
...
impl priv_wnd_events::GuiEvents for BaseWndEvents {
	fn wnd_ty(&self) -> WndTy {
		self.wnd_ty
	}
}

/// This trait is enabled with the `gui` feature, and provides events for
...
/// use winsafe::prelude::*;
/// ```
pub trait GuiEventsWindow: priv_wnd_events::GuiEvents {
...
/// This trait is enabled with the `gui` feature, and provides events for window
...
/// use winsafe::prelude::*;
/// ```
pub trait GuiEventsParent: GuiEventsWindow {
	...
	/// );
	/// ```
	fn wm_command<F>(&self, ctrl_id: impl Into<u16>, code: impl Into<co::CMD>, func: F)
	where
		F: Fn() -> AnyResult<()> + 'static;
	...
	/// );
	/// ```
	fn wm_command_acc_menu<F>(&self, ctrl_id: impl Into<u16> + Copy, func: F)
	where
		F: Fn() -> AnyResult<()> + 'static,
	{
...
impl GuiEventsParent for BaseWndEvents {
	fn wm_command<F>(&self, ctrl_id: impl Into<u16>, code: impl Into<co::CMD>, func: F)
	where
		F: Fn() -> AnyResult<()> + 'static,
	{
