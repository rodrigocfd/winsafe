	...
	pub(in crate::gui) fn show_modal(&self, parent: &impl GuiParent) -> AnyResult<()> {
		...
		*unsafe { &mut *self.0.hchild_prev_focus_parent.get() } =
		...
		let wnd_pos = POINT::new(
			rc_parent.left + (rc_parent.right - rc_parent.left) / 2 - (rc_wnd.right - rc_wnd.left) / 2, // center on parent
			rc_parent.top + (rc_parent.bottom - rc_parent.top) / 2 - (rc_wnd.bottom - rc_wnd.top) / 2,
		);
		...
		self.0.raw_base.create_window(opts.ex_style, atom, Some(&opts.title), opts.style,
			...
			Some(parent.hwnd()), IdMenu::None, &hinst)?;

		self.0.raw_base.base().run_modal_loop(opts.process_dlg_msgs)?; // blocks until window is closed
		Ok(())
	}
}
