...
pub trait shell_IShellLink: ole_IUnknown {
	...
	fn GetShowCmd(&self) -> HrResult<co::SW> {
		...
		ok_to_hrresult(unsafe {
			(vt::<IShellLinkVT>(self).GetShowCmd)(self.ptr(), show_cmd.as_mut())
		})
		.map(|_| show_cmd)
	}
	...
	fn Resolve(&self, hwnd: &HWND, flags: co::SLR) -> HrResult<()> {
		ok_to_hrresult(unsafe {
			(vt::<IShellLinkVT>(self).Resolve)(self.ptr(), hwnd.ptr(), flags.raw())
		})
	}
	...
	fn SetShowCmd(&self, show_cmd: co::SW) -> HrResult<()> {
		ok_to_hrresult(unsafe { (vt::<IShellLinkVT>(self).SetShowCmd)(self.ptr(), show_cmd.raw()) })
	}
