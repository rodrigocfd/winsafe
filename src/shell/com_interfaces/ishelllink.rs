...
pub trait shell_IShellLink: ole_IUnknown {
	...
	/// method.
	#[must_use]
	fn GetPath(&self, fd: Option<&mut WIN32_FIND_DATA>, flags: co::SLGP) -> HrResult<String> {
		let mut buf = WString::new_alloc_buf(MAX_PATH + 1);
		HrRet(unsafe {
	...
	fn GetShowCmd(&self) -> HrResult<co::SW> {
		...
		HrRet(unsafe { (vt::<IShellLinkVT>(self).GetShowCmd)(self.ptr(), show_cmd.as_mut()) })
			.to_hrresult()
			.map(|_| show_cmd)
	}
	...
	fn Resolve(&self, hwnd: &HWND, flags: co::SLR) -> HrResult<()> {
		HrRet(unsafe { (vt::<IShellLinkVT>(self).Resolve)(self.ptr(), hwnd.ptr(), flags.raw()) })
			.to_hrresult()
	}
	...
	fn SetArguments(&self, args: &str) -> HrResult<()> {
		HrRet(unsafe {
			(vt::<IShellLinkVT>(self).SetArguments)(self.ptr(), WString::from_str(args).as_ptr())
		})
		.to_hrresult()
	}
	...
	fn SetDescription(&self, args: &str) -> HrResult<()> {
		HrRet(unsafe {
			(vt::<IShellLinkVT>(self).SetDescription)(self.ptr(), WString::from_str(args).as_ptr())
		})
		.to_hrresult()
	}
	...
	fn SetPath(&self, file: &str) -> HrResult<()> {
		HrRet(unsafe {
			(vt::<IShellLinkVT>(self).SetPath)(self.ptr(), WString::from_str(file).as_ptr())
		})
		.to_hrresult()
	}
	...
	fn SetShowCmd(&self, show_cmd: co::SW) -> HrResult<()> {
		HrRet(unsafe { (vt::<IShellLinkVT>(self).SetShowCmd)(self.ptr(), show_cmd.raw()) })
			.to_hrresult()
	}
