...
pub trait dshow_IMediaFilter: ole_IPersist {
	...
	fn Pause(&self) -> HrResult<bool> {
		okfalse_to_hrresult(unsafe { (vt::<IMediaFilterVT>(self).Pause)(self.ptr()) })
	}
	...
	fn Run(&self, start: i64) -> HrResult<bool> {
		okfalse_to_hrresult(unsafe { (vt::<IMediaFilterVT>(self).Run)(self.ptr(), start) })
	}
	...
	fn Stop(&self) -> HrResult<bool> {
		okfalse_to_hrresult(unsafe { (vt::<IMediaFilterVT>(self).Stop)(self.ptr()) })
	}
}
